...
/* To compare the priority of 2 threads (20161578) */
...
void thread_aging () {
	...
	t_max = list_entry (list_begin (&ready_list), struct thread, elem);
	...
	for (e = list_begin (&ready_list) ; e != list_end (&ready_list) ; e = list_next(e)) {
		...
		if (t->wait_time == 100) {
			...
			if (t->priority > t_max->priority) t_max = t;
		}
	}

	if (t_max -> priority > thread_current()->priority) intr_yield_on_return();
}
