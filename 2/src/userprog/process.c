...
process_execute (const char *file_name) 
{
  ...
  /* parse file name (20161578) */
  char *ptr, *ptr2;
  char parse[128];
  strlcpy (parse, file_name, strlen(file_name) + 1);
  ptr = strtok_r (parse, " ", &ptr2);
  
  if (list_size(&thread_current()->child)==0 && !filesys_open (parse)) return -1;
  // if (!filesys_open (parse)) return -1;

  /* Create a new thread to execute FILE_NAME. */
  //tid = thread_create (file_name, PRI_DEFAULT, start_process, fn_copy);
  tid = thread_create (parse, PRI_DEFAULT, start_process, fn_copy);
  ...
  e = list_begin (&(thread_current() -> child));
	while (e != list_end (&(thread_current() -> child))) {
		...
		if (temp -> exit_status == -1) return process_wait (tid);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* get argc & argv */
  strlcpy (input, file_name, strlen(file_name) + 1);
  ptr = strtok_r (input, " ", &ptr2);
  while (ptr != NULL) {
  ...
  argv_a = (int*) malloc (sizeof (int) * argc);
  strlcpy (input, file_name, strlen(file_name) + 1);
  ptr = strtok_r (input, " ", &ptr2);
  for (i = 0 ; i < argc ; i++) {
	argv[i] = (char*) malloc (sizeof (char) * (strlen(ptr) + 1));
	strlcpy (argv[i], ptr, strlen(ptr) + 1);
	ptr = strtok_r (NULL, " ", &ptr2);
  } 

  /* start pushing to the stack */
